...
class Autoencoder_x_high(nn.Module):
    def __init__(
        self,
        latent_channels=4,
    ...
    def forward(self, x):
        z = self.encode(x, sample_posterior=True)
        recon = self.decode(z)
        return recon
...
class CompositeLoss(nn.Module):
    ...
    def _compute_normals(self, z):
        b, c, h, w = z.shape
        if c != 3:
            raise ValueError(f"Expected 3 channel input, got shape {z.shape}")
        ...
        normals = torch.stack([nx, ny, nz], dim=2)  # [B, 3, 3, H, W]
    ...
    def forward(self, pred, target):
        ...
        if pred.shape[1] != 3:
            raise ValueError(f"Expected tensors with 3 channels, got {pred.shape}")

        l_recon = torch.abs(target - pred).mean()
        abs_err = torch.abs(target - pred)

        if self.use_sobel:
            gx, gy = self._sobel_gradients(abs_err)
        else:
            gx, gy = self._simple_gradients(abs_err)
        ...
        cos_sim = torch.sum(pred_normals * target_normals, dim=2)
        l_norm = (1.0 - cos_sim).mean()

        l_ssim = 1.0 - self._ssim(pred, target, window_size=self.ssim_window_size)
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.amp.autocast(
            ...
            recon = model(x_in)
            total_loss, loss_stats = loss_module(recon, x_in)
        ...
        del x_high, x_med, x_static, x_cond, y_batch, x_in, recon, total_loss, loss_stats
    ...
    if is_ddp:
        metrics_tensor = torch.tensor(
            [running_loss, running_recon, running_grad, running_norm, running_ssim, n_samples],
            device=device,
            dtype=torch.float64,
        )
        ...
        n_samples = int(metrics_tensor[5].item())
...
def main(config: dict | None = None, args=None):
    ...
    data_min = float(np.nanmin(viirs_arr))
    data_max = float(np.nanmax(viirs_arr))
    logger.info(f"x_high min: {data_min:.6f}, max: {data_max:.6f}")

    model = Autoencoder_x_high(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train VAE on x_high (3x1024x1024) with composite loss."
    )
    ...
    parser.add_argument("--cleanup_every", type=int, default=20, help="Memory cleanup every N batches")
    ...
    parser.add_argument(
        "--learnable_lambdas",
        action="store_true",
        help="Learn lambda1/lambda2/lambda3 instead of fixing them"
    )
    parser.add_argument("--no_sobel", action="store_true", help="Disable Sobel gradients and use finite differences")
